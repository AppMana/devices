...
All 19 indexed Watch drawings also have active display envelopes, case dimensions and separately labelled total thickness including the rear sensor. All 22 other indexed accessory/computer drawings have reviewed case/charger/connector/clearance features and contour tables where available: 454 named measurements, 1,290 independent contour ordinates and 62 explicitly labelled ZEISS magnet-center points across 45 drawing pages. Review scopes and unresolved source ambiguities remain explicit; this is not full CAD reconstruction. Earbud keepout regions are not treated as body envelopes; undimensioned remote heights and case depths remain null. Headset dimensions depend on the documented stowed/worn configuration.

Feature review includes magnetic mounting locations/polarity, antenna and material exclusions, local edge/corner profiles, camera/sensor, acoustic, button and connector geometry for all 22 indexed iPad models, 27 indexed iPhones, and 19 indexed Apple Watches. Air M2/M3/M4 and iPad A16/10th-generation shared-applicability drawings are explicitly cited per measurement. `features` preserves nested per-measurement provenance, explicit magnetic segment intervals and partial contour points; `profiles` preserves separately dimensioned local x/y ordinates, which must not be zipped into polygon points. Watch profiles distinguish `labelled-point-table` (explicitly paired source rows) from `independent-ordinate-sequence`; local axes can be x/y or y/z. Dataset coverage counters count per-device measurements, including explicitly shared drawings, rather than claiming unique source callouts or complete CAD reconstruction. Keepout cone angles describe accessory clearance, not measured camera field of view. Combined front camera/sensor keepout centres are separate from individual camera centres, which remain unavailable where not dimensioned.

Unresolved geometry is kept explicit: combined iPhone sensor regions do not locate individual optical centers; Mini rear-camera references conflict with body extents; some magnetic layouts omit individual magnet coordinates; source contour ordinate sets do not define paired curve vertices; and several sheets contain inconsistent or reused labels. These are source limitations, not values to estimate from the PDF image. Each affected record includes the relevant ambiguity and source page.
